...
在 使用 IPO 模组前，首先需要进入“**渠道管理****与****费率管理**”作业，进行一次融资渠道初始化配置 (Whale 团队首次交付会进行一次初始配置),日后劵商若无其他新增融资渠道发生，不需要再额外操作此作业。
...
在系统上线前，会根据港交所要求，系统默认设置好中签费率，券商可点选右上角：**默认中****签****费率**，修改或设定设定对应信息。
